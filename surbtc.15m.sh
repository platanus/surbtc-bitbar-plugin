...
echo "|image=$bitcoin_icon"
echo "---"
echo "BITCOIN $(btc_last_price) |size=14 image=$bitcoin_icon"
preprocess "$btc" "min_ask" "  ASK" "|size=11 color=green"
preprocess "$btc" "max_bid" "  BID" "|size=11 color=red"
preprocess "$btc" "volume" "24hr VOL" "|size=11 color=black"
echo "---"
echo "ETHER $(eth_last_price) |size=14 image=$eth_icon"
preprocess "$eth" "min_ask" "  ASK" "|size=11 color=green"
preprocess "$eth" "max_bid" "  BID" "|size=11 color=red"
preprocess "$eth" "volume" "24hr VOL" "|size=11 color=black"
echo "---"
